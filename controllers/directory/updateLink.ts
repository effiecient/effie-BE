...
export async function updateLink(req: VercelRequest, res: VercelResponse) {
  ...
  let errValidate = await validateBody({ username, path, relativePath, link, title, isPinned, newRelativePath, newPath, publicAccess, personalAccess });
  ...
  if (req.headers.username !== username && linkData.publicAccess !== "editor" && !linkData.personalAccess.some((item: any) => item.username === req.headers.username && item.access === "editor")) {
    res.status(401).json({
      status: STATUS_ERROR,
      message: "Unauthorized.",
    });
